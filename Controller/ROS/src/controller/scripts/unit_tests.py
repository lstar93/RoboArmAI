...
from robot_kinematics import ForwardKinematics, Point, InverseKinematics
...
class forward_kinematics_unittests(unittest.TestCase):
    ...
    def test_prev_to_curr_joint_transform_matrix(self):
        fkine = ForwardKinematics()
        np.testing.assert_array_almost_equal(self.dh_test_arr, fkine.prev_to_curr_joint_transform_matrix(pi/2, 3, 5, pi))
    ...
    def test_fabrik(self):
        ...
        # kinematics engines
        ikine = InverseKinematics(dh_matrix, joints_lengths, robo_arm_joint_limits, first_rev_joint_point)
        fkine = ForwardKinematics()
